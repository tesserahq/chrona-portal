...
export function loader() {
  ...
  const identiesUrl = process.env.IDENTIES_API_URL

  return { apiUrl, nodeEnv, identiesUrl }
}

export default function ProjectMembers() {
  const { apiUrl, nodeEnv, identiesUrl } = useLoaderData<typeof loader>()
  ...
  const fetchInvitations = async () => {
    ...
    try {
      ...
      const workspaceId = getWorkspaceID()
      const response = await fetchApi(
        `${apiUrl}/workspaces/${workspaceId}/invitations`,
        token,
        nodeEnv,
      )
  ...
  const fetchMemberships = async () => {
    ...
    try {
      const token = await getAccessTokenSilently()
      const response = await fetchApi(
        `${apiUrl}/projects/${params.project_id}/memberships`,
        token,
        nodeEnv,
      )
  ...
  return (
    <div className="h-full">
      <div className="mb-5 flex items-center justify-between">
        <h1 className="text-2xl font-bold dark:text-foreground">Member</h1>
              ...
              {workspace?.created_by_id !== member.user_id && (
                <Popover>
                  <PopoverTrigger asChild>
                    {/* its disable for now because we need expose user detail and we have error when remove member */}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const { project_id, invitation_id, membership_id, token, email, type } =
  ...
  try {
    if (request.method === 'DELETE') {
      ...
      if (type === 'member') {
        await fetchApi(
          `${apiUrl}/projects/${project_id}/memberships/${membership_id}`,
          token.toString(),
          nodeEnv,
          {
            method: 'DELETE',
